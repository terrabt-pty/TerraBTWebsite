      ...
      <div style={{ background: "#FFFFFF", minHeight: "100vh" }}>
        ...
        <article style={{ maxWidth: "820px", margin: "0 auto", padding: "40px 24px 80px" }}>
          ...
          <p style={{ color: "#475569", fontSize: "1.125rem", lineHeight: 1.7, marginBottom: "48px", borderBottom: "1px solid #E2E8F0", paddingBottom: "32px" }}>
            "Service key" is SAP's name for what most of us would call an API key. It is a JSON document holding a set of credentials. You get it by running <code style={codeStyle}>cf create-service-key</code>. What sits inside it, and how far it can reach, depends entirely on which service the key belongs to. The BTP cockpit does not show all of them in one place. That is the core problem this post addresses. Below is a service by service breakdown of what sits inside each key and what it can touch. Every claim links to its source.
          </p>
          ...
          <section style={{ marginBottom: "40px" }}>
            ...
            <ServiceCard
              ...
              unlocks="This gives a direct database connection over port 443. HANA Cloud sits on the public internet by default. There is no VPN and no Cloud Connector in between. Access is limited to whatever the bound database user has, nothing more. If that user has wide grants, the reach is wide. If it is scoped to one schema, it stays there. The real problem is usually not the key itself. It is that teams bind a high-privilege user because at setup time it was the faster option."
            ...
            <ServiceCard
              ...
              unlocks="This one is often described incorrectly. The key by itself is only an OAuth client credential. It does not hold the passwords of your backend systems directly. What it does is authenticate against the Destination Service REST API of that subaccount. Calling GET /destination-configuration/v1/destinations returns all destinations registered in the subaccount. If a destination stores a Basic-auth password or an OAuth client secret, the service returns that value in the response. So the key provides a way to read the stored credentials of whatever systems those destinations point to, such as S/4HANA, SuccessFactors, or a third-party API. The key itself does not store those credentials."
            ...
            <ServiceCard
              ...
              unlocks="These are actual hyperscaler credentials. An S3 access key, or the Azure Blob or GCS equivalent, for the customer's bucket. Read, write, delete, list, all of it. This is typically where AI Core grounding data, Build Apps backups, and CAP uploads are kept. One point worth noting. This object-level access through the hyperscaler API does not appear in the BTP Audit Log Service at all. It shows up only in the hyperscaler's own logging, and many teams do not forward that to their SOC."
          ...
          <section style={{ marginBottom: "40px", background: "#F8FAFC", borderRadius: "12px", padding: "32px", border: "1px solid #E2E8F0" }}>
            ...
            <ol style={{ ...p, paddingLeft: "20px", margin: 0 }}>
              <li style={{ marginBottom: "12px" }}>
                <strong style={strong}>Some keys lead to other credentials.</strong> A Destination key does not hold your backend passwords, but the API behind it returns them. One key can take you to many. This is not unique to SAP. An AWS key with Secrets Manager access does the same thing. On BTP, the Destination service makes this the normal pattern, not a misconfiguration.
              </li>
              <li style={{ marginBottom: "12px" }}>
                <strong style={strong}>Some keys are a step toward the on-premise network.</strong> A Connectivity key by itself works only from inside the CF space. Once there is a foothold in a CF app, it lets requests pass through the Cloud Connector tunnel toward on-premise systems. It is one step, not an open door, but it still counts as a step.
              </li>
              <li style={{ marginBottom: "12px" }}>
                <strong style={strong}>They outlive the people who created them.</strong> Removing the user who made a key does nothing to the key. SAP KBA <a href="https://userapps.support.sap.com/sap/support/knowledge/en/3220053" target="_blank" rel="noopener noreferrer" style={a}>3220053</a> covers this related shadow-user behavior. Keys made by people who left the company two years back are still working.
              </li>
              <li style={{ marginBottom: "12px" }}>
              ...
              <li style={{ marginBottom: "12px" }}>
                <strong style={strong}>The audit log keeps only 90 days by default.</strong> The BTP Audit Log Service retains data for 90 days on the free plan. Longer retention requires the premium paid plan. A key created early in a long campaign can have its creation event already aged out before anyone looks.
              </li>
              <li style={{ marginBottom: "12px" }}>
                <strong style={strong}>Some access does not appear in the BTP Audit Log at all.</strong> Object-level access in Object Store goes through the hyperscaler API and shows only in the hyperscaler's logging. Direct SQL access using a HANA key is logged by HANA's own auditing, and only if it is switched on, not by the BTP Audit Log. Watching only the BTP Audit Log means missing both of these.
              </li>
          ...
          <section style={{ marginBottom: "40px" }}>
            ...
            <div style={incidentCard}>
              ...
              <p style={p}>
                No service key caused this. The real point is that credentials of one tenant were reachable from another tenant, because of a flaw in the platform itself. Service keys are one of the things sitting inside that same blast radius.
              </p>
              <p style={smallSrc}>
