...
import { ArrowLeft, ArrowRight, CheckCircle, Brain, FileText, Zap, TrendingUp, Shield, Workflow, BookOpen } from "lucide-react";
...
export default function ClaudeAI() {
  ...
  const benefits = [
    t('aiInvoice.benefits.0'),
    t('aiInvoice.benefits.1'),
    t('aiInvoice.benefits.2'),
    t('aiInvoice.benefits.3'),
    t('aiInvoice.benefits.4')
  ];

  return (
    <div className="min-h-screen flex flex-col">
      <SEOHead
        title={`${t('services.ai.title')} - TerraBT`}
        description={t('services.ai.description')}
        path="/services/claude-ai"
      />
      <Navigation />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="max-w-4xl">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6">
              ...
              <br />
              <span className="text-foreground">{t('aiInvoice.subtitle')}</span>
            </h1>

            <p className="text-xl text-muted-foreground mb-8 leading-relaxed">
              {t('aiInvoice.description')}
            </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-foreground">
              {t('aiInvoice.servicesTitle')}
            </h2>
            <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
              {t('aiInvoice.servicesSubtitle')}
            </p>
          </div>

          <div className="grid md:grid-cols-2 gap-8">
            {services.map((service, index) => {
              const serviceData = t(`aiInvoice.features.${service.key}`, { returnObjects: true }) as { title: string; description: string; items: string[] };
              return (
                <Card
                  key={index}
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-bold mb-6 text-foreground">
                {t('aiInvoice.whyChooseTitle')}
              </h2>
              <p className="text-lg text-muted-foreground mb-8">
                {t('aiInvoice.whyChooseDescription')}
              </p>
            ...
            <Card className="p-8 bg-gradient-to-br from-card to-card/50">
              <div className="space-y-6">
                <div>
                  <div className="text-4xl font-bold text-primary mb-2">{t('aiInvoice.statistics.accuracy.value')}</div>
                  <div className="text-muted-foreground">{t('aiInvoice.statistics.accuracy.label')}</div>
                </div>
                <div>
                  <div className="text-4xl font-bold text-trust mb-2">{t('aiInvoice.statistics.timeReduction.value')}</div>
                  <div className="text-muted-foreground">{t('aiInvoice.statistics.timeReduction.label')}</div>
                </div>
                <div>
                  <div className="text-4xl font-bold text-primary mb-2">{t('aiInvoice.statistics.costSavings.value')}</div>
                  <div className="text-muted-foreground">{t('aiInvoice.statistics.costSavings.label')}</div>
                </div>
                <div>
                  <div className="text-4xl font-bold text-trust mb-2">{t('aiInvoice.statistics.availability.value')}</div>
                  <div className="text-muted-foreground">{t('aiInvoice.statistics.availability.label')}</div>
                </div>
              </div>
            </Card>
          </div>
          ...
          <Card className="p-8 border-primary/20 bg-gradient-to-r from-primary/5 to-trust/5">
            <div className="flex flex-col md:flex-row items-center justify-between gap-8">
              <div className="flex-1">
                ...
                <h3 className="text-2xl font-bold mb-2 text-foreground">{t('blog.post3.title')}</h3>
                <p className="text-muted-foreground mb-4 md:mb-0 max-w-2xl">{t('blog.post3.excerpt')}</p>
              </div>
              <Link href={getLocalizedPath("/blog/claude-ai-invoice-processing")}>
